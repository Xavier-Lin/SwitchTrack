...
class Trainer(object):
  def __init__(self, opt, model, optimizer=None):
    ...
    self.model_with_loss = DDP(
      self.model_with_loss.cuda(opt.local_rank), device_ids=[self.opt.local_rank],find_unused_parameters=True)
    
    # Optimizer to device
    for state in self.optimizer.state.values():
      for k, v in state.items():
        if isinstance(v, torch.Tensor):
          state[k] = v.cuda(device=self.opt.local_rank, non_blocking=True)
