...
class DeepSort(object):
    def __init__(
        self, max_dist=0.1, min_confidence=0.3, nms_max_overlap=1.0, 
        ...
        self.m_a = max_age
        self.n_i = n_init
        self.extractor = Extractor(model_path=None, use_cuda=True)
    
    def reset(self,):
        metric = NearestNeighborDistanceMetric(
            "cosine", self.max_cosine_distance, self.n_b)
        self.tracker = Tracker(
            metric, max_iou_distance=self.max_iou_dist, max_age=self.m_a, n_init=self.n_i)
        self.tracks = []

    def step(self, output_results, ori_img):
        '''
        results = [{'bbox':, 'ct':,  'class':,  'tracking':, 'reid':, 'score':,} {}, ...]
        '''
        ...
        for d in output_results:
            box.append(d['bbox'])# xyxy
            feat.append(d['reid'])  
            s.append(d['score'])          
        # import pdb;pdb.set_trace()
        confidences = np.array(s, dtype=np.float32)
        ...
        # bboxes /= scale
        # bbox_xyxy = bboxes
        bbox_tlwh = self._xyxy_to_tlwh_array(bbox_xyxy.reshape(-1, 4))
        remain_inds = confidences > self.min_confidence
        bbox_tlwh = bbox_tlwh[remain_inds]
        confidences = confidences[remain_inds]

        # generate detections
        _ = self._get_features(bbox_tlwh, ori_img)# only used for testing speed
