...
from utils.match import iou_distance, linear_assignment, fuse_score_iou
...
class BYTETracker(object):
    ...
    def step(self, output_results):
        ...
        for d in output_results:
            b.append(d['bbox'])#xyxy
            s.append(d['score'])
        scores = np.array(s, dtype=np.float32)
        bboxes = np.array(b, dtype=np.float32)# x1y1x2y2
        ...
        dists = fuse_score_iou(dists, detections)
        ...
        matches, u_track, u_detection_second = linear_assignment(dists, thresh=0.4)
        for itracked, idet in matches:
