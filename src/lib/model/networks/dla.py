...
class BasicBlock(nn.Module):
    ...
    def fuseforward(self, x, residual=None):
        if residual is None:
            residual = x
...
class Tree(nn.Module):
    ...
    def fuseforward(self, x, residual=None, children=None):
        if self.training and residual is not None:
            x = x + residual.sum() * 0.0
        children = [] if children is None else children
        bottom = self.downsample(x) if self.downsample else x
        residual = self.project[0](bottom) if self.project else bottom
        if self.level_root:
            children.append(bottom)
        x1 = self.tree1(x, residual)
        if self.levels == 1:
...
class DLA(nn.Module):
    ...
    def fuseforward(self, x, pre_img=None, pre_hm=None):
        y = []
        x = self.base_layer[-1](self.base_layer[0](x))
        if pre_img is not None:
            x = x + self.pre_img_layer[-1](self.pre_img_layer[0](pre_img))
        if pre_hm is not None:
            x = x + self.pre_hm_layer[-1](self.pre_hm_layer[0](pre_hm))
        for i in range(6):
            if (i == 0) or (i == 1):
                x = getattr(self, 'level{}'.format(i))[-1](getattr(self, 'level{}'.format(i))[0](x))
            else:
                x = getattr(self, 'level{}'.format(i))(x)
            y.append(x)
...
def dla34(pretrained=True, **kwargs):  # DLA-34
    ...
    if pretrained:
        model.load_pretrained_model(
            data='../pretrain/', name='dla34-ba72cf86.pth')
    else:
        lg.info('Warning: No ImageNet pretrain!!')
    return model
...
class DLASeg(BaseModel):
    def __init__(self, num_layers, heads, head_convs, opt):
        ...
        if opt.local_rank in [-1,0]:
            lg.info('Using node type: {}'.format(self.node_type))
        self.first_level = int(np.log2(down_ratio))#2
        self.last_level = 5
        self.base = globals()['{}'.format(opt.arch)](
