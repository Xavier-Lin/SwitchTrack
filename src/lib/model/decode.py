...
def generic_decode(output, K=100, opt=None):
  #[{'hm':(b,1,h/4,w/4),'reg':(b,2,h/4,w/4),'ltrb_amodal':(b,4,h/4,w/4),'tracking':(b,2,h/4,w/4)，‘pre_inds’:}]
  # K 200 ,  opt
  heat = output['hm']
  ...
  if 'reg' in output:#T
    reg = output['reg'] #  b，2，h/4，w/4 --- d_x， d_y
    reg = _tranpose_and_gather_feat(reg, inds)
    #网络输出的回归预测 b,2,h/4,w/4---d_x,d_y  bz,100  --- b 100 2 
    reg = reg.view(batch, K, 2)# b,100,2
    xs = xs0.view(batch, K, 1) + reg[:, :, 0:1]
    ys = ys0.view(batch, K, 1) + reg[:, :, 1:2]
    ret['ct_acc'] = torch.cat([xs, ys], dim=2)
  else:
  ...
  if 'pre_inds' in output and output['pre_inds'] is not None:
    ...
    ret['pre_cts'] = torch.cat(
      [pre_xs.unsqueeze(2), pre_ys.unsqueeze(2)], dim=2)# 1 num 2  
  
  if 'reid' in output and output['reid'] is not None: 
    id_feature = output['reid']
    id_feature = _tranpose_and_gather_feat(id_feature, inds)
    # proposal --- b k 128
    id_feature = F.normalize(id_feature, dim=-1)
    ret['reid_feature'] = id_feature
